...
export default function RoleWithDialog({
  ...
  return (
    <div className={`flex items-end ${direction === DIRECTIONS[RIGHT] && 'flex-row-reverse'}`}>
      ...
      <div className={`relative grow py-[10px] px-[12px] md:py-[20px] md:pl-[70px] md:pr-[50px] border ${roleBorderColors[roleName]}`}>
        <AnimatedText
        ...
        />
        <span className={`absolute inline-block px-[16px] py-[4px] top-[10px] left-[-6px] ${roleBgColors[roleName]}`}>{roleName}</span>
      </div>
    </div>
  );
}
